    ...
    function fn_object_to_html(array $elements, $parentId = 0, $defaultParent = 0) {
      ...
      foreach ($elements as $element) {
        ...
        if ($element->post_parent == $parentId) {
          ...
          $html .= "<li ".($children ? 'class="has-child"' : '')."><a href='".get_permalink($element->ID)."'>{$element->post_title}". ($element->post_parent == $defaultParent ? '<span class=\'dropdown show\'>+</span><span class=\'dropdown hide hidden\'>-</span>' : '')."</a>";
